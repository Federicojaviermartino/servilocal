    ...
    <header className="sticky top-0 z-50 border-b border-gray-200 bg-white" role="banner">
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6" aria-label="Navegación principal">
        ...
        <div className="hidden items-center gap-6 md:flex">
          <Link href="/services/search" className="flex items-center gap-1 text-sm text-gray-600 transition-colors hover:text-primary-500">
          ...
          {isAuthenticated && user ? (
            <div className="flex items-center gap-4">
              ...
              <Link href="/dashboard/profile" className="flex items-center gap-1 text-sm text-gray-600 hover:text-primary-500">
      ...
      {menuOpen && (
        <div id="mobile-menu" className="border-t border-gray-200 bg-white px-4 py-4 md:hidden" role="navigation" aria-label="Menú móvil">
          <div className="flex flex-col gap-3">
            <Link href="/services/search" className="text-sm text-gray-600" onClick={() => setMenuOpen(false)}>
              Buscar servicios
            </Link>
            {isAuthenticated && user ? (
              <>
                ...
                <Link href="/dashboard/profile" className="text-sm text-gray-600" onClick={() => setMenuOpen(false)}>
                  Mi perfil
                </Link>
